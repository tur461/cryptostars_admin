      ...
      <div className="col container_swapwrap__header liquidity_default_header">
        <div className="row">
          <div className="col liquidity_default_header__left">
            <h2>Liquidity</h2>
            <p>Add liquidity to receive CRONOS-LP tokens</p>
            <Link to="#" onClick={() => handleClick(2)} className="btn">
              Add Liquidity
            </Link>
          </div>
      ...
      <div className="col">
        ...
        {!isUserConnected ? (
          <div className="col addLiquidity_info_card">
            <p>
              Connect to a wallet to view your <br />
              liquidity.
            </p>
          </div>
            ...
            {userLpTokens.map((lp, index) => (
              <div className="liquidityList" key={index}>
                <div
                  className={
                    currentIndex === index
                      ? "liquidityListMain openbox"
                      : "liquidityListMain"
                  }
                  onClick={() => toggleDropdwon(index)}
                >
                  <img src={lp.token0Obj.icon} alt="icon" />{" "}
                  <img src={lp.token1Obj.icon} alt="icon" />
                  {lp.pairName}
                </div>
                {currentIndex === index && (
                  <ul>
                    <li>
                      Pooled {lp.token0Obj.symbol}:{" "}
                      <span>
                        <TokenBalance
                          address={lp.token0}
                          balance={lp.token0Deposit}
                        />{" "}
                        <img src={lp.token0Obj.icon} alt="icon" />
                      </span>
                    </li>
                    <li>
                      Pooled {lp.token1Obj.symbol}:{" "}
                      <span>
                        <TokenBalance
                          address={lp.token1}
                          balance={lp.token1Deposit}
                        />{" "}
                        <img src={lp.token1Obj.icon} alt="icon" />
                      </span>
                    </li>
                    <li>
                      Your pool tokens: <span>{lp.balance.toFixed(5)}</span>
                    </li>
                    <li>
                      Your pool share: <span>{lp.poolShare}% </span>
                    </li>
                    <li>
                      <Link
                        className="btn"
        ...
        <div className="col addLiquidity_info_instruction">
          ...
          <p>
            {" "}
            {/* Or, if you staked your ANCHOR-LP tokens in a farm, unstake them to
            see them here. */}
          </p>
        </div>
      </div>
